...
function normalizeCart(cart=[]){ return Array.isArray(cart)?cart.filter(x=>getProduct(x.id)&&Number(x.qty)>0).map(x=>({id:x.id,qty:Math.min(99,Math.max(1,Math.floor(Number(x.qty))))})):[]; }
...
function setCartQty(cart,id,qty){ if(qty<=0)return removeFromCart(cart,id); return normalizeCart(cart).map(x=>x.id===id?{...x,qty:Math.min(99,Math.floor(qty))}:x); }
function removeFromCart(cart,id){ return normalizeCart(cart).filter(x=>x.id!==id); }
