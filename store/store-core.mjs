...
export function formatMoney(amount,locale="en"){ return new Intl.NumberFormat(locale==="ar"?"ar-SA":"en-SA",{style:"currency",currency:"SAR",maximumFractionDigits:0}).format(Number(amount)||0); }
export function normalizeCart(cart=[]){ return Array.isArray(cart)?cart.filter(x=>getProduct(x.id)&&Number(x.qty)>0).map(x=>({id:x.id,qty:Math.min(99,Math.max(1,Math.floor(Number(x.qty))))})):[]; }
export function addToCart(cart,id,qty=1){ const next=normalizeCart(cart); const raw=typeof id==="object"?id.id:id; const canonical=raw==="web"?"web-launch":raw; const item=next.find(x=>x.id===canonical); if(!getProduct(canonical)) return next; if(item)item.qty=Math.min(99,item.qty+Math.max(1,Math.floor(qty))); else next.push({id:canonical,qty:Math.max(1,Math.floor(qty))}); return next; }
export function setCartQty(cart,id,qty){ if(qty<=0)return removeFromCart(cart,id); return normalizeCart(cart).map(x=>x.id===id?{...x,qty:Math.min(99,Math.floor(qty))}:x); }
export function removeFromCart(cart,id){ return normalizeCart(cart).filter(x=>x.id!==id); }
export function cartSubtotal(cart){ return normalizeCart(cart).reduce((s,x)=>s+getProduct(x.id).price*x.qty,0); }
export function calculateTotals(cart,{vatRate=.15,discount=0}={}){ const subtotal=cartSubtotal(cart); const safeDiscount=Math.min(subtotal,Math.max(0,Number(discount)||0)); const taxable=subtotal-safeDiscount; const vat=Math.round(taxable*Math.max(0,Number(vatRate)||0)); return {subtotal,discount:safeDiscount,vat,total:taxable+vat}; }
export function validateCheckout(v={}){ const errors={}; if(!String(v.name||"").trim())errors.name="required"; if(!/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(String(v.email||"")))errors.email="invalid"; if(!String(v.phone||"").trim())errors.phone="required"; if(!String(v.city||"").trim())errors.city="required"; return {valid:Object.keys(errors).length===0,errors}; }
export function localeRoute(locale,path=""){ const p=String(path).replace(/^\/+/,""); return locale==="ar"?`/ar/store/${p}`:`/store/${p}`; }

export function cartTotal(cart){ return cartSubtotal(cart); }
